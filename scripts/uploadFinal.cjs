...
function httpsPost(hostname, httpPath, body, headers) {
  return new Promise((resolve, reject) => {
    const options = { hostname, path: httpPath, method: 'POST', headers };
...
function httpsPatch(hostname, httpPath, body, headers) {
  return new Promise((resolve, reject) => {
    const options = { hostname, path: httpPath, method: 'PATCH', headers };
    const req = https.request(options, (res) => {
      let data = '';
      res.on('data', (c) => data += c);
      res.on('end', () => resolve({ status: res.statusCode || 0, data }));
...
function uploadToGCS(localPath, filename, contentType, accessToken) {
  return new Promise((resolve, reject) => {
...
async function makePublic(filename, accessToken) {
...
async function updateFirestore(productId, imageUrl, accessToken) {
